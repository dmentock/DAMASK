...
int main(int argc, char *argv[]) {
  ...
  if (!load_path.empty()) {
    config.load_steps = config.parse_load_yaml(load_path, config.fields);
  } else {
    throw std::runtime_error("Tension file not specified.");
  }

  if (!grid_path.empty()) {
    config.vti_file_content = config.read_file(grid_path);
  } else {
  ...
  Spectral spectral;
  spectral.init(0, grid_, config.numerics.fftw_planner_flag, config.numerics.fftw_timelimit);
